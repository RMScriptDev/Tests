...
VisualsGroup:AddToggle('EspToggle', {Text = 'Enable ESP', Default = false, Callback = function(v) getgenv().espEnabled = v end})
...
task.spawn(function()
    while true do
        if getgenv().DeleteMinesEnabled then
            ...
            if mf then
                local count = 0
                for _, obj in ipairs(mf:GetChildren()) do obj:Destroy(); count = count + 1 end
...
if s then
    ...
    if ow then
        ...
        local function getTarget(origin)
            ...
            local best, bestDist = nil, (getgenv().IgnoreFOV and 9e9 or getgenv().fov)
            local mousePos = getgenv().IgnoreFOV and nil or UIS:GetMouseLocation()
                ...
                if tPart then
                    ...
                    if getgenv().wallcheck then
                        ...
                        if not canSee then
                            ...
                            if alt then canSee = isVisible(alt, origin); if canSee then tPart = alt end
                        end
                    end
                    if canSee then
                        local dist
                        if getgenv().IgnoreFOV then
                            dist = (origin - tPart.Position).Magnitude
                        else
                            local pos, on = cam:WorldToViewportPoint(tPart.Position)
                            if not on then continue end
                            dist = (Vector2.new(pos.X, pos.Y) - mousePos).Magnitude
                        end
                        if dist < bestDist then best, bestDist = tPart, dist end
                    end
                end
            end
            return best
        ...
        local function updateTargetsCache()
            ...
            for _, char in next, ow:GetChildren() do
                if char:IsA("Model") and not char:FindFirstChildOfClass("ForceField") then
                    ...
                    if not z or z.Health > 0 then table.insert(newList, char) end
                end
            end
        ...
        local oldCast = hookfunction(rawget(ac, "new"), newcclosure(function(a, origin, direction, maxDist, ...)
        ...
        if getgenv().ItemsESPEnabled then
            ...
            if folder then
                for _, child in pairs(folder:GetDescendants()) do
                    if child:IsA("BasePart") or child:IsA("Model") then applyItemHighlight(child) end
                end
            end
        else
...
Library:Notify('Script loaded (Landmine ESP removed). Press End to open menu.', 5)
print('Arbyzware Tools (without Landmine ESP) loaded.')
