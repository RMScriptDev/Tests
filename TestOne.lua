...
local function createPlayerBillboard(playerModel)
    if not playerModel then return nil end
    
    local attachPart = playerModel:FindFirstChild("HumanoidRootPart") or playerModel:FindFirstChild("Head") or playerModel:FindFirstChild("Torso")
...
local function addPlayerHighlight(playerModel)
    ...
    activeHighlights["Player_" .. playerModel.Name] = playerHighlights[playerModel]
    
    local billboard, healthBar, healthText = createPlayerBillboard(playerModel)
    if billboard then
        playerBillboards[playerModel] = {
            billboard = billboard,
            healthBar = healthBar,
...
local function removePlayerHighlight(playerModel)
    if playerHighlights[playerModel] then
        destroyHighlights(playerHighlights[playerModel])
        playerHighlights[playerModel] = nil
        activeHighlights["Player_" .. playerModel.Name] = nil
    end
    if playerBillboards[playerModel] then
        pcall(function() playerBillboards[playerModel].billboard:Destroy() end)
        playerBillboards[playerModel] = nil
    end
end
...
local function setupPlayerESP()
    if playerEspEnabled then
        ...
        table.insert(activeConnections, workspaceAddedConn)
        
        local workspaceRemovingConn = workspace.DescendantRemoving:Connect(function(descendant)
            if playerEspEnabled and descendant:IsA("Model") and playerHighlights[descendant] then
                removePlayerHighlight(descendant)
            end
        end)
        table.insert(activeConnections, workspaceRemovingConn)
        ...
        playerHighlights = {}
        for playerModel, billboardData in pairs(playerBillboards) do
...
local function togglePlayerESP()
    ...
    Rayfield:Notify({
        Title = "Player ESP",
        Content = playerEspEnabled and "ON (цвет по HP)" or "OFF",
        Duration = 2
    })
    if playerEspEnabled then
        setupPlayerESP()
...
local function addMonsterHighlight(monster)
    ...
    activeHighlights["Monster_" .. monster.Name] = monsterHighlights[monster]
end
...
local ButtonMonsterESP = VisualsTab:CreateButton({
   ...
   Callback = function()
       ...
       Rayfield:Notify({
           Title = "Monster ESP",
           Content = monsterEnabled and "ON" or "OFF",
           Duration = 2
       })
       if monsterEnabled then
           setupMonsterESP()
...
local ButtonStalkerESP = VisualsTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({
            Title = "Stalker ESP",
            Content = stalkerEnabled and "ON" or "OFF",
            Duration = 2
        })
        if stalkerEnabled then
            setupStalkerESP()
...
local function addSpiderHighlight(spider)
    ...
    activeHighlights["Spider_" .. spider.Name] = spiderHighlights[spider]
end
...
local ButtonSpiderESP = VisualsTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({
            Title = "Spider ESP",
            Content = spiderEnabled and "ON" or "OFF",
            Duration = 2
        })
        if spiderEnabled then
            setupSpiderESP()
...
local ButtonFullbright = VisualsTab:CreateButton({
   ...
   Callback = function()
       ...
       Rayfield:Notify({
           Title = "Fullbright",
           Content = fullbrightEnabled and "ON" or "OFF",
           Duration = 2
       })
...
local function miscNotify(title, content, duration)
    Rayfield:Notify({ Title = title, Content = content, Duration = duration or 2.5 })
end
    ...
    for _, descendant in pairs(workspace:GetDescendants()) do
        if descendant:IsA("Model") and isMonster(descendant) and not trackedMonsters[descendant.Name] then
            ...
            if descendant.Name == "Intruder" then
                miscNotify("Intruder in closet", "", 3)
            else
                miscNotify("Monster Spawned", "", 3)
            end
        end
    end
end
...
local function startMonsterTracking()
    ...
    local removingConn = workspace.DescendantRemoving:Connect(function(descendant)
        if monsterTrackingEnabled and descendant:IsA("Model") and isMonster(descendant) and trackedMonsters[descendant.Name] then
            ...
            miscNotify("Monster Despawned", "", 3)
        end
    end)
    
    monsterTrackingThread = {addedConn, removingConn}
    miscNotify("Monster Notify", "Tracking ENABLED", 2)
end
...
local function stopMonsterTracking()
    ...
    trackedMonsters = {}
    miscNotify("Monster Notify", "Tracking DISABLED", 2)
end

local function toggleMonsterNotify()
    if monsterTrackingEnabled then stopMonsterTracking() else startMonsterTracking() end
end

MiscTab:CreateButton({
    Name = "Monster Notify",
    Callback = toggleMonsterNotify
})
    ...
    Callback = function()
        local success = pcall(function()
            ...
            if statusUI then
                ...
                if foodBG then foodBG.Visible = false end
            end
            local replicatedStorage = game:GetService("ReplicatedStorage")
            local gameState = replicatedStorage:FindFirstChild("GameState")
            if gameState then
                local weirdStrict = gameState:FindFirstChild("WeirdStrict")
                if weirdStrict then weirdStrict.Value = false end
                local totalModifiers = gameState:FindFirstChild("TotalModifiers")
                if totalModifiers then totalModifiers.Value = 0 end
            end
        end)
        if success then miscNotify("Infinity Hunger", "Enabled", 2) else miscNotify("Infinity Hunger", "Failed", 2) end
    end
})
    ...
    Callback = function()
        local success = pcall(function()
            ...
            if doors then
                ...
                miscNotify("Barriers", "Removed " .. removed .. " closet barriers", 2)
            else
                miscNotify("Barriers", "Doors folder not found", 2)
            end
        end)
        if not success then miscNotify("Barriers", "Failed", 2) end
    end
})
...
MiscTab:CreateButton({
    Name = "Infinity Yield",
    Callback = function()
        miscNotify("Infinity Yield", "Loading...", 2)
        local success, err = pcall(function()
            loadstring(game:HttpGet('https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source'))()
        end)
        if success then miscNotify("Infinity Yield", "Loaded!", 2) else miscNotify("Infinity Yield", "Error: " .. tostring(err), 3) end
    end
})
...
-- Storage for temporarily removed cabin objects
local hiddenCabins = {}
...
-- Hide specific cabin objects
local function hideCabins()
    local cabins = workspace:FindFirstChild("Cabins")
    if not cabins then
        miscNotify("Spider Shoot", "Cabins folder not found, skipping object removal", 2)
        return false
    end
    
    -- Targets: index 4, Cabin by name, index 2, index 3
    local targets = {
        cabins:GetChildren()[4],
        cabins:FindFirstChild("Cabin"),
...
-- Restore previously hidden cabin objects
...
-- Main action: teleport, rotate, shoot, wait, return, restore cabins
local function spiderShootAction()
    local player = game.Players.LocalPlayer
    local character = player.Character
    if not character or not character:FindFirstChild("HumanoidRootPart") then
        miscNotify("Spider Shoot", "Character not found", 2)
        return
    end

    local spider = findSpider()
    if not spider then
        miscNotify("Spider Shoot", "No spider found! (WorkerHead/Spider/Arachnid)", 3)
        return
    end

    local spiderHead = getSpiderHead(spider)
    if not spiderHead then
        miscNotify("Spider Shoot", "Spider has no aim part", 2)
        return
    end

    -- Hide cabins before teleport
    hideCabins()
    task.wait(0.2)

    -- Save original position, rotation, camera
    local hrp = character.HumanoidRootPart
    local originalCF = hrp.CFrame
    local camera = workspace.CurrentCamera
    local originalCameraCF = camera.CFrame

    -- Teleport and rotate player to face spider
    local teleportPos = spiderHead.Position + spiderHead.CFrame.LookVector * -2.5
    hrp.CFrame = CFrame.new(teleportPos, spiderHead.Position)

    -- Snap camera to spider
    ...
    if remote then remote = remote:FindFirstChild("Shoot") end
    if not remote then remote = game:GetService("ReplicatedStorage"):FindFirstChild("Shoot") end
    ...
    if tool then
        shootSuccess = pcall(function() remote:FireServer(tool) end)
    else
        shootSuccess = pcall(function() remote:FireServer() end)
    end

    if shootSuccess then
        miscNotify("Spider Shoot", "Shot fired at spider!", 2)
    else
        miscNotify("Spider Shoot", "Failed to shoot", 2)
    end

    -- Wait 2 seconds (adjustable)
    task.wait(2)
...
-- Keybind for Spider Shoot (default: Y)
MiscTab:CreateKeybind({
    Name = "Spider Shoot Keybind",
    CurrentKeybind = "Y",
    HoldToInteract = false,
    Flag = "SpiderShootBind",
    Callback = function()
        miscNotify("⚠️ Warning", "Use only if there's a spider on the window!", 3)
        task.wait(0.5)
        spiderShootAction()
...
MiscTab:CreateButton({
    Name = "Info",
    Callback = function()
        miscNotify("Info", "Script ZOVCOPTER by NAGIEV", 5)
    end
})
